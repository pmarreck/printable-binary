...
- **File Container (`.pbf.json`)**: Wrap a file as one self-describing, self-verifying JSON object that preserves the filename and a CRC-32 check â decodable by every implementation and resistant to whitespace-mangling transports (email, reflow); see [File Container](#file-container-pbfjson)
...
- **Monospace-safe glyph set:** every glyph is vetted to occupy the same width in common monospace fonts, so alignment in editors/terminals/diffs stays intact (surprisingly many Unicode symbols donât).
...
- **Control characters are labeled**: Bytes 0â31 and DEL render as mnemonic symbols (`â`, `â§`, `â«`, etc.), making structure and control flow obvious without extra tooling.
- **Trade-off**: Hex expands data by exactly 2Ã in bytes. PrintableBinary averages about 1.8â1.9Ã on real-world binaries (thanks to the many 1- and 2-byte UTF-8 mappings) and only approaches 3Ã in the worst case. The small extra cost buys markedly better readability and paste safety.

## Performance

All implementations share the same byteâglyph map and pass the same test suite; they differ only in language/runtime. Indicative throughput â encode + decode of 10 MB of random `/dev/urandom` data via `test/benchmark_test`, release builds on Apple Silicon:
...
- **Flat character map**: a comptime-built contiguous byte buffer (~1.5 KB) replacing 256 scattered fat pointers â fits in L1 cache.
...
The C and Lua decoders were tuned in a measured pass: C uses direct 1-/2-byte lookup tables (**1.9Ã** decode). Lua got two passes â resolving each glyph by its UTF-8 leading-byte length (instead of brute-forcing all four), then writing decoded bytes straight into a LuaJIT `string.buffer` via its FFI `reserve`/`commit` API (no per-byte `string.char`). Together that took Lua **decode from ~8 to ~91 MB/s** â now faster than its own encode (which uses `string.buffer:put`, ~1.2Ã). Every optimization is benchmarked before and after (hyperfine), and a continuous memory-leak suite (`test/leak_test`) guards the FFI/C paths against regressions.

The optional PGO path (`make pgo-ffi`) adds a further ~1â3% via profile-guided branch layout, dogfooding the Zig library through its public C ABI.
...
echo -n "Hello,â£Worldï¹" | ./bin/printable-binary -d
# Output: Hello, World!
...
- Two explicit modes, selected by tabs:
  - **Encode** â drag-and-drop or browse to encode any file. Choose plain-text
  ...
  - **Decode** â **paste** a printable-binary block *or* a `.pbf.json` container,
...
- Large outputs skip the textarea to avoid browser jank â use the Copy/Download
...
- To hack locally, serve the repo root over HTTP (any static file server) and open
  `index.html` â the ES-module import and character-map fetch need `http://`, not
  `file://`; no build step required.

### File Container (`.pbf.json`)

The `-C`/`--container` flag wraps a file as a self-describing
**`printable-binary-file.json`** â one JSON object that keeps the filename and a
...
`crc32_encoded` is verified over the canonical (whitespace-stripped) payload â
with a lenient JSON parse for hard line-wraps â so a container survives being
...
every other byte is shown as an uppercase hex run prefixed by `ÎÏ` (Greek
Omicron+Chi). It keeps embedded text fully legible while making binary regions
explicit â useful when you care more about reading the ASCII than about density.
...
print(encoded)  -- Output: Hello,â£World!
...
Those commands show whichever map is active. To override the defaults, place a `character_map.txt` next to the executable (or set `PRINTABLE_BINARY_MAP`) and rerun the same flags to confirm your changes. The file format is simple: **256 lines of UTF-8, one glyph per byte value starting at 0x00**. No commas, spaces, or indexesâjust the literal characters in order. After editing, run `./utils/audit_character_map.lua character_map.txt` (and `./utils/update_eaw_data.sh` when Unicode publishes a new width table) plus `./utils/generate_embedded_map.lua` so the embedded headers stay in sync.
...
If none of those locations exist, the embedded table is used automatically. Edit the file to experiment with alternative glyphsâthe LuaJIT, C, Node.js, and WebAssembly implementations will all honor the override on their next run.

### Environment Variables

PrintableBinary respects a couple of environment variables across every implementation (LuaJIT, C, WASM, Node, and tests):

- `PRINTABLE_BINARY_MAP` â absolute or relative path to a `character_map.txt` that overrides the embedded table. The lookup order is described above.
- `PRINTABLE_BINARY_MUTE_STATS` â set to `1`, `true`, or `yes` to suppress the usual "Encoded â¦" / "Decoding mode â¦" statistics that are normally written to stderr. This is handy for scripts that expect clean stderr output while still reusing the default behavior interactively.
...
- **Escape-proof JSON embed** â Avoid backslash/quote hell by pre-encoding the bytes, then drop them straight into a JSON string:
...
- **Bash assertion on binary snippets** â Keep fixtures inline without here-doc escaping. Generate the encoded blob once (e.g., `PRINTABLE_BINARY_MUTE_STATS=1 printf 'CAFÃ\n' | ./bin/printable-binary`), then paste it into the here-doc:
...
- **Peek mixed binary/text streams in place** â Mirror a live HTTP POST while keeping the raw bytes intact:
...
- **Web page embed + JS decode** â Ship binary in HTML as plain text, then revive it in the browser using the shared module:
...
- **Inspect hint bytes of common formats** â Spot magic numbers without a hex viewer:

  ```bash
  head -c 16 some.pdf | ./bin/printable-binary
  # Expect to see %PDFâ£1.7â¦ rendered directly.

  head -c 8 image.png | ./bin/printable-binary
  # Should show 89PNGââ£â£ if the PNG signature is intact.
  ```

### Piping to UTF-16 (or other text encodings)

The encoded output is UTF-8 text. Because every glyph is in the BMP, it transcodes **losslessly** to UTF-16 â handy for Windows/PowerShell/JavaScript/.NET consumers. Two thin `iconv` wrappers default to **little-endian** (ARM and x86_64) and handle the BOM:
...
### â **Excellent Compatibility With:**

- **JSON** - Perfect in quoted strings (we re-encode `"` as `Ë®`)
...
- **C/C++/Java/etc.** - Perfect in string literals (we re-encode `\` as `â§¹`)
- **Shell scripts** - Perfect in quoted strings (we re-encode `'` as `Ê¼`)
- **SQL** - Perfect in quoted strings
- **Most UTF-8 aware text formats**

### ð¯ **Key Design Decisions for Compatibility:**

- **Double quotes** (34) â `Ë®` (U+02EE) - Avoids JSON/XML attribute conflicts
- **Single quotes** (39) â `Ê¼` (U+02BC) - Avoids shell/SQL conflicts
- **Backslashes** (92) â `â§¹` (U+29F9) - Avoids escape sequence issues
- **Control characters** â Safe Unicode symbols (Â·, Â¶, â, etc.)
- **No problematic delimiters** in our special encodings

### ð **Usage Recommendations:**
...
1. **Byte Economy** - Prefer shorter UTF-8 sequences (1-2 bytes) where possible to minimize encoding overhead. Most printable ASCII passes through at 1:1, so text-heavy data expands minimally; pure random binary averages ~1.85Ã.

2. **Visual Suggestion** - Each glyph should hint at what it replaces. Examples:
   - `â£` (open box) for space - clearly indicates "there's a space here"
   - `â` for carriage return - universal "return/enter" symbol
   - `â¥` for tab - arrow pointing to a bar suggests tabulation
   - `ËË` for angle brackets - similar shape, clearly related

3. **Unambiguous Distinction** - The glyph must *not* be confused with the original character. This explains choices like:
   - `Ë®` for double-quote - modifier letter double apostrophe, visually evocative and only 2 bytes
   - `Ê¼` for single-quote - modifier letter apostrophe looks similar but is clearly distinct
   - `â§·` for backslash - has a horizontal stroke through it

**High-byte ordering (0x80-0xFF):** The extended byte mappings are roughly lexically ordered - they begin with variants of A (Ä, Ä, Ç...) and end with variants of Z (Åº, Å¹, Å¾, Å½, Å¼, Å»). This allows developers to ballpark approximately what byte value is being represented just by glancing at the glyph's base letter.

## Character Encoding

- **Control Characters (0-31)**: Mapped to visually distinct symbols like Â·, Â¯, Â«, Â», Âµ, etc.
- **Space (32)**: Encoded as â£ for visibility
- **Shell-unsafe ASCII characters**: Mapped to safe Unicode alternatives:
  - Exclamation mark (33) â ï¹ (U+FE57) Small Exclamation Mark
  - Double quote (34) â Ë® (U+02EE) Modifier Letter Double Apostrophe
  - Hash (35) â â¯ (U+266F) Music Sharp Sign
  - Dollar sign (36) â ï¹© (U+FE69) Small Dollar Sign
  - Percent (37) â ï¹ª (U+FE6A) Small Percent Sign
  - Ampersand (38) â â (U+214B) Turned Ampersand
  - Single quote (39) â Ê¼ (U+02BC) Modifier Letter Apostrophe
  - Parentheses (40-41) â â¨â© (U+2768-2769) Medium Parenthesis Ornaments
  - Asterisk (42) â ï¹¡ (U+FE61) Small Asterisk
  - Plus (43) â ï¹¢ (U+FE62) Small Plus Sign
  - Minus (45) â ï¹£ (U+FE63) Small Hyphen-Minus
  - Slash (47) â â (U+2044) Fraction Slash
  - Colon (58) â ê (U+A789) Modifier Letter Colon
  - Semicolon (59) â Í¾ (U+037E) Greek Question Mark
  - Equals (61) â ê (U+A78A) Modifier Letter Short Equals Sign
  - Question mark (63) â É (U+0241) Latin Capital Letter Glottal Stop
  - At sign (64) â @ (U+0040) Commercial At
  - Backslash (92) â â§· (U+29F7) Reverse Solidus with Horizontal Stroke
  - Brackets (91, 93) â â¦â§ (U+27E6-27E7) Mathematical White Square Brackets
  - Backtick (96) â Ë (U+02CB) Modifier Letter Grave Accent
  - Braces (123-125) â â´â£âµ (Ornament and mathematical variants)
  - Tilde (126) â Ë (U+02DC) Small Tilde
- **DEL (127)**: Encoded as â¦
...
| --- | --- | --- | --- | --- |
| 0 | Â· | U+00B7 | C2 B7 | Middle Dot |
| 1 | Â¯ | U+00AF | C2 AF | Macron |
| 2 | Â« | U+00AB | C2 AB | Left-Pointing Double Angle Quotation Mark |
| 3 | Â» | U+00BB | C2 BB | Right-Pointing Double Angle Quotation Mark |
| 4 | Ï | U+03DF | CF 9F | Greek Small Letter Koppa |
| 5 | Â¿ | U+00BF | C2 BF | Inverted Question Mark |
| 6 | Â¡ | U+00A1 | C2 A1 | Inverted Exclamation Mark |
| 7 | Âª | U+00AA | C2 AA | Feminine Ordinal Indicator |
| 8 | â« | U+232B | E2 8C AB | Erase To The Left |
| 9 | â¥ | U+21E5 | E2 87 A5 | Rightwards Arrow To Bar |
| 10 | Â¶ | U+00B6 | C2 B6 | Pilcrow Sign |
| 11 | â§ | U+21A7 | E2 86 A7 | Downwards Arrow From Bar |
| 12 | Â§ | U+00A7 | C2 A7 | Section Sign |
| 13 | â | U+23CE | E2 8F 8E | Return Symbol |
| 14 | È¯ | U+022F | C8 AF | Latin Small Letter O With Dot Above |
| 15 | Ê | U+0298 | CA 98 | Latin Letter Bilabial Click |
| 16 | Æ | U+0194 | C6 94 | Latin Capital Letter Gamma |
| 17 | Â¹ | U+00B9 | C2 B9 | Superscript One |
| 18 | Â² | U+00B2 | C2 B2 | Superscript Two |
| 19 | Âº | U+00BA | C2 BA | Masculine Ordinal Indicator |
